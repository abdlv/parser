...
def get_title(soup, title_cut):
    ...
    attribute = parts[1]
    value = parts[2]
    return soup.find(tag, {attribute: value}).text
...
def insert_news(link, title, content):
    res_id = 0
    nd_date = 0
    s_date = 0
    not_date = "2022-04-23"
...
def main():
    ...
    for instruction in instructions:
        ...
        for news_url in news_urls:
            ...
            print('----------------')
            print()
            args.append((news_url, news_title, news_date, news_text))
            insert_news(news_url, news_title, news_text)
